...
#define TIME_UPDATE_FREQ (TIME_UPDATE_FREQ_SEC*1000)
...
static void ICACHE_FLASH_ATTR periodic_time_check(void) {
	ntp_get_time();
#ifdef PLATFORM_DEBUG
			ets_uart_printf("I've requested time, will set up the timer now for %d seconds\r\n", TIME_UPDATE_FREQ_SEC);
#endif
	os_timer_disarm(&TimeChecker);
	os_timer_setfn(&TimeChecker, (os_timer_func_t *)wifi_connect, NULL);
	os_timer_arm(&TimeChecker, TIME_UPDATE_FREQ, 0);
}
...
static void ICACHE_FLASH_ATTR wifi_check_ip(void *arg)
{
	...
	case STATION_GOT_IP:
		...
		if(ipConfig.ip.addr != 0) {
			...
			ets_uart_printf("Requesting NTP time...\r\n");
#endif
			periodic_time_check();
			return;
		}
...
#ifdef PLATFORM_DEBUG
	ets_uart_printf("Initiating WiFi connection.\r\n");
#endif
...
void ICACHE_FLASH_ATTR configure_uart() {

	uart_init(BIT_RATE_9600, 0);

	//SET_PERI_REG_MASK(UART_CONF0(1), BIT(22)); //invert uart
}
...
void ICACHE_FLASH_ATTR configure_wifi() {
	...
	if(wifi_station_get_auto_connect() != WIFI_AUTO_RECONNECT)
		wifi_station_set_auto_connect(1);
...
void ICACHE_FLASH_ATTR user_init()
{
	...
	os_timer_disarm(&TimeChecker);
	os_timer_setfn(&TimeChecker, (os_timer_func_t *)wifi_check_ip, (void *)0);
	os_timer_arm(&TimeChecker, 1000, 1);
